...
class IdentitasMenawarProyekSearch extends IdentitasMenawarProyek
{
    ...
    {
        return [
            [['id', 'identitas', 'proyek', 'waktu'], 'integer'],
            [['dana', 'pesan', 'file', 'created_at'], 'safe'],
        ];
    }
     ...
     */
    public function search($params, $kondisi)
    {
        if (empty($kondisi)) {
            $query = IdentitasMenawarProyek::find();
        }
        else{
        $query = IdentitasMenawarProyek::find()->where(['proyek'=>$kondisi]);
        }
